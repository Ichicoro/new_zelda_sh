...
type PageEnum = "home" | "blog" | "projects" | "photos";
...
const linkArray: LinkType[] = [
  ...
  { name: "Projects", path: "/projects", activePage: "projects", active: false },
...
const Header: React.FunctionComponent<Props> = ({ activePath, showTitle = true, dots }) => {
  return <div className="flex flex-col justify-center items-center md:items-start">
    {showTitle && <h1 className="font-sans leading-tight">ZELDA&apos;S LAIR</h1>}
    <div className={`font-germania-one font-bold text-2xl flex flex-row md:flex-col gap-3 md:gap-0 mb-2`}>
      {linkArray.map((link) => {
        const isActivePath = link.activePage === activePath;

        return link.active !== false
          ? <div key={link.name} className="flex flex-row gap-1">
            {dots !== undefined && isActivePath ? <div className="block md:hidden text-center h-1">·</div> : null}
            <Link
            ...
            {dots !== undefined && isActivePath ? <div className="block md:hidden text-center h-1">·</div> : null}
            {dots !== undefined && isActivePath ? <div className="hidden md:block text-center h-1">···</div> : null}
          </div>
          : <span
            key={link.name}
